...
[VPS上docker部署教程，点击前往](https://github.com/Kiritocyz/Argo-Nezha-Service-Container/blob/main/README.md#vps-%E9%83%A8%E7%BD%B2%E6%96%B9%E5%BC%8F-1-----docker)
 ...
 下面这个是v1v2版本哪吒兼容v0,但目前改变量会报错，不用改就按它配置来
  ...
  V1V0版：  docker镜像：v1vo镜像 `hxyus/nezha-argov1vo:latest`， `mikehand888/argo-nezha:latest`支持 amd64 和 arm64 架构。
...
| :--- | :--- | :--- | :--- | :--- |
| **1** | **`GH_USER`** | 必须填写 *(你的 GitHub 账号)* | 必须填写 *(只用于数据自动备份)* | 告诉系统谁在操作备份 |
| **2** | **`GH_CLIENTID`** | **必须填写** *(GitHub 申请的 ID)* | *可完全不填 / 删掉* | v0 的唯一登录认证方式 |
| **3** | **`GH_CLIENTSECRET`** | **必须填写** *(GitHub 申请的密钥)* | *可完全不填 / 删掉* | v0 的唯一登录认证方式 |
| **4** | **`GH_PAT`** | 必须填写 *(你的 GitHub Token)* | 必须填写 *(只用于数据自动备份)* | 备份时推送到你私库的钥匙 |
| **5** | **`ARGO_DOMAIN`** | 必须填写 *(你的访问域名)* | 必须填写 *(你的访问域名)* | 面板暴露的公网自定义域名 |
| **6** | **`ARGO_AUTH`** | 必须填写 *(你的 cf 穿透凭证)* | 必须填写 *(你的 cf 穿透凭证)* | 穿透回源的凭证 |
| **7** | **`GH_REPO`** | 必须填写 *(用户名/新仓库名)* | 必须填写 *(只用于数据自动备份)* | 存放你备份数据的仓库 |
| **8** | **`NZ_VERSION`** | `v0` *(或不填)* | `v1` | **核心控制开关：告诉容器去拉哪个版本的面板** |
| **10** | `DASHBOARD_VERSION` | `v1.1.1` (或填 `v2.x.y`) | **版本控制**：明确锁定部署新版（v1/v2系列）面板。 |
| **11** | `uuid` | `6529b007-521e-4340-8903-87c8344bcfcb` | **自动生成argo节点**：可以在容器的日志说明找到节点订阅链接如：https://xx.xxx.com/6529b007-521e-4340-8903-87c8344bcfcb |

| **12** | **`NZ_USER`** | *可完全不填 / 删掉* | **必须填写** *(如 `admin`)* | **v1 自定义管理员账号** |
| **13** | **`NZ_PASSWORD`** | *可完全不填 / 删掉* | **必须填写** *(如 `password123`)* | **v1 自定义管理员密码** |                      
