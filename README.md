...
docker镜像：v1vo镜像 `hxyus/nezha-argov1vo:latest`， `mikehand888/argo-nezha:latest`支持 amd64 和 arm64 架构。
...
[VPS上docker部署教程，点击前往](https://github.com/Kiritocyz/Argo-Nezha-Service-Container/blob/main/README.md#vps-%E9%83%A8%E7%BD%B2%E6%96%B9%E5%BC%8F-1-----docker)
 ...
 下面这个是v1v2版本哪吒兼容v0,但目前改变量会报错，不用改就按它配置来
  ...
  V1V0版：  docker镜像：v1vo镜像 `hxyus/nezha-argov1vo:latest`， `fscarmen/argo-nezha:latest`支持 amd64 和 arm64 架构。
...
| **13** | `uuid` | `6529b007-521e-4340-8903-87c8344bcfcb` | **自动生成argo节点**：可以在容器的日志说明找到节点订阅链接如：https://xx.xxx.com/6529b007-521e-4340-8903-87c8344bcfcb |
...
| 序号 | 变量名称 (Key) | 建议填入的值 (Value) | 作用与填写解释 |
| :---: | :--- | :--- | :--- |
| 1 | **`GH_USER`** | `你的GitHub用户名` | **必须填写**：指定执行自动定时备份数据的 GitHub 账号名称。 |
| 2 | **`GH_CLIENTID`** | `你的GitHub Client ID` | **必须填写**：你在 GitHub 申请的 OAuth 应用的 Client ID，用于后台登录。 |
| 3 | **`GH_CLIENTSECRET`** | `你的GitHub Client Secret` | **必须填写**：你在 GitHub 申请的 OAuth 应用的密钥，用于后台登录。 |
| 4 | **`GH_PAT`** | `你的GitHub Token (ghp_...)` | **必须填写**：具有 `repo` 读写权限的个人访问令牌，用于往你的私库推送备份。 |
| 5 | **`GH_REPO`** | `你的备份私库名字` | **必须填写**：你在 GitHub 专门新建用来存放哪吒备份数据的仓库名。 |
| 6 | **`ARGO_DOMAIN`** | `://yourdomain.com` | **必须填写**：哪吒面板暴露的公网自定义访问域名（需在 CF 提前解析）。 |
| 7 | **`ARGO_AUTH`** | `你的CF隧道Token或JSON` | **必须填写**：Cloudflare Tunnel 穿透回源的密钥凭证。 |
| 8 | **`NZ_VERSION`** | `v0` | **大版本锁**：强制让容器内部脚本只运行老版本哪吒逻辑。 |
| 9 | **`DASHBOARD_VERSION`** | `v0.20.13` | **精确版本锁**：将哪吒面板主程序死死定格在 v0 时代的最终养老稳定版。 |
| 10 | **`NO_AUTO_RENEW`** | `1` | **更新总水阀**：彻底切断清晨 4 点联网下载行为，防止因上游脚本变动导致格式不兼容。 |
| 11 | **`UUID`** | `一串自定义的UUID密钥` | *可选填*：填入即在容器后台额外免费收获一个不限速的 VLESS/VMess 代理节点。 |                      
